...
instruction1: T_WRITE expression ';' { $$ = createOperatorNode(OPER_WRITE,1,$2); }

skip_instruction : T_SKIP { $$ = createOperatorNode(OPER_SKIP,0); }
	;

expression:T_NUMERIC { $$ = createNumericNode($1); }
	|expression '+' expression { $$ = createOperatorNode(OPER_ADD,2,$1,$3); }
	|expression '-' expression { $$ = createOperatorNode(OPER_SUB,2,$1,$3); }
	|expression '*' expression { $$ = createOperatorNode(OPER_MULT,2,$1,$3); }
	|expression '/' expression { $$ = createOperatorNode(OPER_DIV,2,$1,$3); }
